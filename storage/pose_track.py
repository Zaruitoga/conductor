...
HEADER = struct.Struct("<8sB3xdd")
RECORD = struct.Struct("<d7f")
...
assert DTYPE.itemsize == RECORD.size
...
class PoseTrackWriter:
    ...
    def __init__(self, path: str, wheel_R: float, wheel_r: float,
        ...
        self._file.write(HEADER.pack(MAGIC, 0, wheel_R, wheel_r))
        self._file.flush()
    ...
    def close(self) -> None:
        ...
        self._file.flush()
        self._file.seek(0)
        self._file.write(HEADER.pack(MAGIC, FLAG_COMPLETE, self.wheel_R, self.wheel_r))
        self._file.flush()
        self._file.close()
        self._file = None
...
class TrackHeader:
    ...
    __slots__ = ("wheel_R", "wheel_r", "complete", "records", "size")
...
def read_header(path: str) -> TrackHeader | None:
    ...
    magic, flags, wheel_R, wheel_r = HEADER.unpack(raw)
    if magic != MAGIC:
    ...
    records = max(0, (size - HEADER.size) // RECORD.size)
    return TrackHeader(wheel_R, wheel_r, bool(flags & FLAG_COMPLETE), records, size)
...
def read_array(path: str, header: TrackHeader | None = None) -> np.ndarray:
    ...
    return np.fromfile(path, dtype=DTYPE, count=header.records, offset=HEADER.size)
...
class PoseTrackService:
    ...
    def read(self, session: str, take: str, *, start: float | None = None,
        ...
        out["poses"] = ({c: [] for c in COLUMNS} if header is None else
    ...
    async def ensure(self, session: str, take: str) -> dict:
        ...
        track is readable as it fills.  Everything up to `_tasks[key] = …` runs
        without awaiting, which is what makes "one producer" true — two
        concurrent callers cannot both find the slot empty.
        """
        key  = (session, take)
        if key in self._tasks:
            return self.status(session, take)
        ...
        if key in self._errors:
            ...
            # told instead, and a deliberate retry means removing the file.
            return self.status(session, take)
        ...
        self._tasks[key] = asyncio.ensure_future(self._compute(key, csv_path, path))
        return self.status(session, take)
    ...
    async def _compute(self, key: tuple[str, str], csv_path: str, out_path: str) -> None:
        try:
            ...
            # The error sticks, so `ensure` stops retrying: what fails here is a
            # deterministic read of a file that will not have changed by the
            # next poll, and a 4 Hz panel would otherwise spawn a doomed thread
            # four times a second for as long as the take stayed on screen.
    ...
    def cancel_all(self) -> None:
        """Stop every running computation — used on shutdown."""
    ...
    def _header(self, key: tuple[str, str], path: str) -> TrackHeader | None:
        """`read_header`, with an unreadable file turned into a sticky error."""
        try:
    ...
    def _duration(path: str, header: TrackHeader | None) -> float:
        ...
        if header is None or header.records == 0:
            return 0.0
        offset = HEADER.size + (header.records - 1) * RECORD.size
        with open(path, "rb") as f:
            f.seek(offset)
            raw = f.read(RECORD.size)
        if len(raw) < RECORD.size:
            return 0.0
        return round(RECORD.unpack(raw)[0], 6)
