...
- **The state pill is derived from stored data alone**: no `video_file` ⇒ « sans vidéo » ; anchors absent ⇒ « non aligné » ; both ⇒ « aligné ». Changing take does not change the URL (`?session=&take=` is read at boot and never rewritten).
