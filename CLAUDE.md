...
- Shortcuts are **reimplemented on the page**, like `api/viz/viz.js` reimplemented its `api()` helper: `↑`/`↓` cycle candidates (bringing one into view when zoomed), `Espace` plays/pauses, suppressed while typing and under any modifier. The playhead only runs on an **aligned** take, since the two anchors are what tie the timelines — until then the note says so instead of drawing a cursor that would mean nothing.
