...
The playback progress bar is still **read-only in the panel** — `POST /api/playback/seek` exists (see "Resuming at an instant") but no control is wired to it yet; that comes with the sweep. Pause/resume state is always read back from `playback.paused`, never applied optimistically.
...
During a replay the take's video plays in an inset in the corner of the scene, slave to **`frame.t`** and never to `playback.elapsed_s` (4 Hz rounded to a tenth — 72° of wheel at two turns a second). One layout, deliberately: the three switchable ones and the scene dressing are a separate ticket. `video.js` is the wiring (the element, the file, the permanent rVFC chain, the bar); `sync-clock.js` is the whole of the behaviour, DOM-free apart from the one `<video>` it drives, and lifted from `proto/8-video-dans-le-viz` — the only piece of that prototype worth keeping.
