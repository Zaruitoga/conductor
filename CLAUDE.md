...
- **`video.js` holds the rVFC discipline** the prototype paid for, and #27's frame stepping lands there: one permanent `requestVideoFrameCallback` chain (never a callback armed per seek — an abandoned one arrives late and the *next* seek reads the frame before), requests in `currentTime` and readings in `mediaTime` **never summed**, and one pending seek slot, latest wins. `media` is the single source of position, playing or paused. A browser where rVFC never fires (embedded webviews do this) is detected once after `loadeddata` and falls back to polling `currentTime`, which the HUD names rather than hiding.
- Shortcuts are **reimplemented on the page**, like `api/viz/viz.js` reimplemented its `api()` helper: `↑`/`↓` cycle candidates (bringing one into view when zoomed), `Espace` plays/pauses, suppressed while typing and under any modifier. **`↑` goes to the *next* candidate**, i.e. later in the take: the candidates are ordered in time and marked on a timeline, so the axis they answer to is the curve's, not a list's — #27's key map still reads « précédent / suivant » and wants updating. The playhead only runs on an **aligned** take, since the two anchors are what tie the timelines — until then the note says so instead of drawing a cursor that would mean nothing.

The discoverability links from the panel and the viz, the frame stepping, the pinned-frame comparator and the confirmation belong to #27.
