...
A **third page**, served from `api/align/` and mounted at `/align/` — the same recipe as `/viz/`, and the same trap: the root mount is a **catch-all**, so a page registered after it is not a 404, it is handed the panel's `index.html` with a 200 and the right content type, and the failure surfaces several layers later on a script the panel never asked for. `tests/test_surfaces.py` pins the ordering for all three surfaces. No build, no bundler, no CDN: raw ES modules, `align.js` + `curve.js` + `video.js`, offline like the rest.
