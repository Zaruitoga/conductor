...
- **A step never computes where the next frame is; it walks until the reading *changes*.** `_one(dir)` moves away from the last request by δ until the reported PTS moves *in the direction asked* — that change, never the value reached, is the answer, so whatever constant sits between the domains the frame reached is a neighbour. This is not a hypothetical: measured on the reference rush, a request at 5.000 s displays PTS 4.929633 and the next two land 33.311 / 33.312 ms apart — the request-to-reading gap is ~54 ms, **more than a whole frame**, so these very files are the case where "`mediaTime` + an interval" aims beside the mark. Keypresses stack in a bounded queue and drain one frame at a time, never merged into a jump.
- **A stride finds *a* neighbour; only a bisection finds the *nearest* one.** A stride locates the first boundary on its own lattice, so a **presentation window narrower than δ falls between two probes** and its frame is unreachable — from either side, always the same one. Reported from the page: take 003's frame 91, `90 → 92 → 90`. Those rushes are B-frame reordered (decode order ≠ presentation order) and their `stts` alternates 66.6 ms deltas with **9/90000 s**; measured in Chrome, two consecutive request windows there ran 42 ms and 25 ms where the PTS grid says a flat 33.3. So `_one` now **brackets** with a plain stride and then **halves** the bracket (`BISECT_S`, `BISECT_MAX`) until the boundary itself is found — still only ever asking the stride's question, at shrinking distances, and still placing nothing. It is done on **every** step: the tempting gate (did the reading move by more than one interval?) is wrong precisely here, since passing over a narrow frame moves the reading by *that frame's* few milliseconds, i.e. *less* than an interval. Nothing distinguishes "landed on the neighbour" from "jumped one" without probing between, which is what the halving is. Costs ~12 seeks per keypress against three; `BISECT_S` is the only lever, and it trades a frame's reachability for probes.
- **One request-domain distance, `_ctFrame`** — one frame, measured once, and what `jump()` (⇧, ten frames) multiplies. There used to be a second, adaptive one beside it (`_ctStep`, where a forward probe *started*) and it cost two defects. Conflated with `_ctFrame` it took ⇧ from ten frames to five, silently: a backward step leaves the cursor a hair inside the previous frame, so the next forward step succeeds at once and writes the shorter distance back — and walking back and forth *is* comparing against the pinned frame. Split off, it still opened every forward walk with a leap, which is exactly what let a narrow window be stepped over. The bracket-and-bisect wants no head start, so it is gone. `_harness.js` pins both.
...
- **`_harness.js` is a development bench and deliberately outside `tests/run.py`** — that suite is Python and dependency-free, this needs a browser. Run it from the page's console: `(await import('/align/_harness.js')).run()`. It substitutes a fake `<video>` (a grid of frames, slow seeks, late presentations, and **an offset between the two domains**) because the path that matters is not exercisable where one usually looks: an embedded webview, and any hidden document, never presents a frame, so rVFC never fires and the page falls back to `currentTime`. Two corrections were shipped blind before it named the defect. It is worth most for what it **reproduces**: on a −0.05 s offset the version that added the domains burns 29 seeks without moving. Twenty-three cases, all green — and it earned its place, catching six real defects here: a cadence measurement that folded the domain offset into its first interval, a HUD announcing « cadence mesurée » when nothing had been (the fallback is 30 fps, exactly what the rushes are, so the wrong label would never have looked wrong), ⇧ decaying to five frames, the two `_still()` faults above, and the unreachable narrow frame.

**A case that passes the first time it is run has asserted nothing**, and three separate times here the first draft did exactly that. The way to check one bites is to point `run()` at a copy of `video.js` with the fix reverted — `run('./_video_sans_correctif.js')` — and this is not ceremony: the `_still()` cases passed against the broken code, and *making them bite* is what exposed that keying the wait off `paused` skipped the whole window. The narrow-window cases likewise passed at first, for a reason worth keeping: δ is a **quarter of a frame**, so on a regular synthetic grid every fourth probe lands exactly on a boundary and a narrow window inserted at a whole number of frames is hit by construction. It has to be placed **off the lattice** — where a real one falls — which is what `withNarrow()` does. Note also that `FakeVideo.pause()` models the in-flight frames and `play()` is **bounded**: an unbounded presentation loop does not fail a case, it hangs every case after it, which reads as the bench being broken rather than the code.
